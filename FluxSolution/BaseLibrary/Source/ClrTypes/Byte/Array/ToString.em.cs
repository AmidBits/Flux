...
namespace Flux
{
  public static partial class XtendByteArray
  {
    ...
    public static string ToDecimalString(this byte[] source, string separator = @" ")
      => string.Join(separator, source.Select(b => b.ToString(System.Globalization.CultureInfo.CurrentCulture)));
    ...
    public static string ToHexString(this byte[] source, string separator = @" ")
      => string.Join(separator, source.Select(b => b.ToString(@"X2", System.Globalization.CultureInfo.CurrentCulture).PadLeft(2, '0')));
  }
}
