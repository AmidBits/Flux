...
namespace Flux.Data
{
  public static class SqlSys
  {
    ...
    public static string SelectColumns(TsqlName tableName3, params string[] columnNames)
      => $"SELECT {(columnNames.Any() ? Tsql.Combine(columnNames) : @"C.*")} FROM sys.schemas S (NOLOCK) INNER JOIN sys.tables AS T (NOLOCK) ON (S.[schema_id] = T.[schema_id]) INNER JOIN sys.columns AS C (NOLOCK) ON (T.[object_id] = C.[object_id]) INNER JOIN sys.types CT (NOLOCK) ON (C.system_type_id = CT.system_type_id AND C.user_type_id = CT.user_type_id) WHERE DB_NAME() = '{tableName3.DatabaseName}' AND S.[name] = '{tableName3.SchemaName}' AND T.[name] = '{tableName3.ObjectName}' ORDER BY [column_id]";
    ...
    public static string SelectTables(TsqlName tableName3, params string[] columnNames)
      => $"SELECT {(columnNames.Any() ? Tsql.Combine(columnNames) : @"T.*")} FROM sys.schemas S (NOLOCK) INNER JOIN sys.tables AS T (NOLOCK) ON (S.[schema_id] = T.[schema_id]) WHERE DB_NAME() = '{tableName3.DatabaseName}' AND S.[name] = '{tableName3.SchemaName}' AND T.[name] = '{tableName3.ObjectName}'";
  }
}
