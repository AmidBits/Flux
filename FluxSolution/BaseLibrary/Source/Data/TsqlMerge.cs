...
namespace Flux.Data
{
  public static class TsqlMerge
  {
    ...
    public static string IntoTarget(TsqlName targetTable3) => IntoTarget(targetTable3, @"*");
    public static string UsingSource(TsqlName sourceTable3, params string[] sourceColumnNames) => $"USING ( SELECT {string.Join(",", sourceColumnNames.Select(name => $"[{name}]"))} FROM {sourceTable3.QualifiedNameQuoted(3)} ) AS S ";
    public static string UsingSource(TsqlName sourceTable3) => UsingSource(sourceTable3, @"*");
    public static string On(params TsqlColumnDefinition[] columnDefinitions) => On(columnDefinitions.Select(cd => cd.ColumnName).ToArray(), columnDefinitions.Select(cd => cd.ColumnName).ToArray(), columnDefinitions.Select(cd => cd.ToString()).ToArray());
    ...
    public static string OnCompareEquality(string sourceColumnName, string targetColumnName, string dataTypeName, bool allowDBNull)
    {
      ...
      static string OnCompareCast(string toType, string columnLeft, string columnRight, bool isNullable) => isNullable ? $"(({OnCompareCast(toType, columnLeft, columnRight, false)}) OR ({Tsql.And(Tsql.IsNull(columnLeft), Tsql.IsNull(columnRight))}))" : Tsql.Equal(Tsql.Cast(columnLeft, toType), Tsql.Cast(columnRight, toType));
      static string OnCompareGeneric(string columnLeft, string columnRight, bool isNullable) => isNullable ? $"(({OnCompareGeneric(columnLeft, columnRight, false)}) OR ({Tsql.And(Tsql.IsNull(columnLeft), Tsql.IsNull(columnRight))}))" : Tsql.Equal(columnLeft, columnRight);
      static string OnCompareGeo(string columnLeft, string columnRight, bool isNullable) => isNullable ? $"(({OnCompareGeo(columnLeft, columnRight, false)}) OR ({Tsql.And(Tsql.IsNull(columnLeft), Tsql.IsNull(columnRight))}))" : $"({columnLeft}.STEquals({columnRight}) = 1)";
