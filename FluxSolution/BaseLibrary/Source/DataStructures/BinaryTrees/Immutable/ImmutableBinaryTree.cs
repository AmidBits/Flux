namespace Flux.DataStructures.BinaryTrees.Immutable
{
  /// <summary>
  ...
  public sealed class ImmutableBinaryTree<TValue>
    : IBinaryTree<TValue>
  {
    public static IBinaryTree<TValue> Empty { get; } = new EmptyBinaryTree();
    ...
    /// <returns></returns>
    public static DataStructures.BinaryTrees.IBinaryTree<TValue> Create(TValue[] inOrder, TValue[] levelOrder)
    {
      return ConstructTree(ImmutableBinaryTree<TValue>.Empty, levelOrder, inOrder, 0, inOrder.Length - 1);

      static DataStructures.BinaryTrees.IBinaryTree<TValue> ConstructTree(Flux.DataStructures.BinaryTrees.IBinaryTree<TValue> startNode, TValue[] levelOrder, TValue[] inOrder, int inStart, int inEnd)
      {
        if (inStart > inEnd)
