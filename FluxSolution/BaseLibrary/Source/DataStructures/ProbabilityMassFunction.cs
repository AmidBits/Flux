namespace Flux
{
  ...
  namespace DataStructures
  {
      ...
      where TProbability : System.Numerics.IFloatingPoint<TProbability>
    {
      ...
      public static ProbabilityMassFunction<TKey, TProbability> Create<TFrequency>(Histogram<TKey, TFrequency> histogram, TProbability factor)
        where TFrequency : System.Numerics.IBinaryInteger<TFrequency>
      {
        ...
        foreach (var item in histogram.ThrowIfNull())
          pmf.m_data[item.Key] = TProbability.CreateChecked(item.Value) / totalFrequencies * factor;
