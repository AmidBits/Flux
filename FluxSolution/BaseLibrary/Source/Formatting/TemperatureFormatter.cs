namespace Flux.Formatting
{
    ...
    : AFormatter
  {
    ...
    {
      if (!string.IsNullOrWhiteSpace(format) && arg is Temperature temperature)
      {
          ...
          if (m.Groups[@"Unit"] is var g0 && g0.Success && g0.Value is var unitString)
          {
            ...
            foreach (var unit in (TemperatureUnit[])System.Enum.GetValues(typeof(TemperatureUnit)))
              if (unit.ToString().StartsWith(unitString, System.StringComparison.InvariantCultureIgnoreCase))
                return string.Format(null, $"{{0:N{decimalPlaces}}}", temperature.ToUnitValue(unit)) + unit.GetUnitSymbol();
          }
        }
      }
