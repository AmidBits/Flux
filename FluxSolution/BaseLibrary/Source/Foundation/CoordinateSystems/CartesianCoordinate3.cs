namespace Flux
{
  ...
  namespace CoordinateSystems
  {
      ...
      : System.IEquatable<CartesianCoordinate3>
    {
      ...
      public static CartesianCoordinate3 InterpolateCubic(CartesianCoordinate3 source, CartesianCoordinate3 target, CartesianCoordinate3 preSource, CartesianCoordinate3 postTarget, double mu)
      {
        var mu2 = mu * mu;
      ...
      /// <summary>Creates a new vector by interpolating between the specified vectors and a unit interval [0, 1].</summary>
      public static CartesianCoordinate3 InterpolateHermite(CartesianCoordinate3 source, CartesianCoordinate3 target, CartesianCoordinate3 preSource, CartesianCoordinate3 postTarget, double mu, double tension, double bias)
      {
        var mu2 = mu * mu;
        var mu3 = mu2 * mu;
      ...
      {
        var dot = System.Math.Clamp(DotProduct(source, target), -1.0, 1.0); // Ensure precision doesn't exceed acos limits.
        ...
        return new CartesianCoordinate3(source.m_x * cos + (target.m_x - source.m_x) * dot * sin, source.m_y * cos + (target.m_y - source.m_y) * dot * sin, source.m_z * cos + (target.m_z - source.m_z) * dot * sin);
      }
