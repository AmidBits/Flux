...
namespace Flux.DataStructures.Graph
{
  ...
  public class OpenGraph<TKey, TWeight>
    : IGraph<TKey, TWeight>
    where TKey : System.IEquatable<TKey>
    where TWeight : System.IEquatable<TWeight>
  {
    private readonly System.Collections.Generic.List<TKey> m_vertices = new System.Collections.Generic.List<TKey>();
    ...
    public void AddDirectedEdge(TKey source, TKey target, TWeight weight)
      => SetDirectedEdge(source, target, weight);
    public void AddUndirectedEdge(TKey source, TKey target, TWeight weight)
      => SetUndirectedEdge(source, target, weight);

    public bool ContainsDirectedEdge(TKey source, TKey target, TWeight weight)
    ...
    public bool ContainsUndirectedEdge(TKey source, TKey target, TWeight weight)
      => m_vertices.IndexOf(source) is var sourceIndex && sourceIndex > -1
      && m_vertices.IndexOf(target) is var targetIndex && targetIndex > -1
      && m_weights[sourceIndex, targetIndex].Equals(weight)
      && m_weights[targetIndex, sourceIndex].Equals(weight);

    public bool RemoveDirectedEdge(TKey source, TKey target, TWeight weight)
    => ResetDirectedEdge(source, target, weight);
    public bool RemoveUndirectedEdge(TKey source, TKey target, TWeight weight)
      => ResetUndirectedEdge(source, target, weight);

    public bool ResetDirectedEdge(TKey source, TKey target, TWeight weight)
    {
    ...
    public bool ResetUndirectedEdge(TKey source, TKey target, TWeight weight)
    {
      var sourceIndex = m_vertices.IndexOf(source);
      var targetIndex = m_vertices.IndexOf(target);
    ...
    public void SetDirectedEdge(TKey source, TKey target, TWeight weight)
    {
      AddVertex(source);
      AddVertex(target);
    ...
    public void SetUndirectedEdge(TKey source, TKey target, TWeight weight)
    {
      AddVertex(source);
      AddVertex(target);
    ...
    public System.Collections.Generic.IEnumerable<Vertex<TKey>> GetVertices()
    {
      for (var row = 0; row < m_vertices.Count; row++)
      {
        ...
        yield return new Vertex<TKey>(m_vertices[row], degree);
      }
    }

    public System.Collections.Generic.IEnumerable<Edge<TKey, TWeight>> GetEdges()
    {
      ...
      for (var row = 0; row < m_vertices.Count; row++)
        for (var column = 0; column < m_vertices.Count; column++)
          if (m_weights[row, column] is var weight && !weight.Equals(default!))
            yield return new Edge<TKey, TWeight>(vertices[row], vertices[column], weight);
    }
    ...
    public override string ToString()
    {
      ...
      sb.Insert(0, $"<{nameof(AdjacentMatrix<TKey, TWeight>)}: ({GetVertices().Count()} vertices, {index} edges)>{System.Environment.NewLine}");
      return sb.ToString();
    }
  }
}
