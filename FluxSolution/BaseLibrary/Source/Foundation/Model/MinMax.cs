...
namespace Flux.Model.TicTacToe2
{
  public struct Move
  {
    public GridCoordinate2 Point { get; }
    ...
    {
      Point = new GridCoordinate2(column, row);
  ...
  public sealed class Game
  {
    ...
    public static bool TryGetMove(int[,] board, GridCoordinate2 location, out int maxScore, out int minScore)
    {
    ...
    public static System.Collections.Generic.List<Move> GetMoves(int[,] board)
    {
      ...
      for (var r = rows - 1; r >= 0; r--)
        for (var c = columns - 1; c >= 0; c--)
          if (TryGetMove(board, new GridCoordinate2(c, r), out var maxScore, out var minScore))
            moves.Add(new Move(r, c, maxScore, minScore));
      return moves;
    }
