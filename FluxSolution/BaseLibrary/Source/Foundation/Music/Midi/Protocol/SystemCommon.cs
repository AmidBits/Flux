namespace Flux.Music.Midi.Protocol
{
  ...
  public sealed class SystemCommon
  {
    ...
    public static byte[] MtcQuarterFrame(int type, int value)
      => new byte[] { (byte)SystemCommonStatus.MidiTimeCodeQuarterFrame, (byte)((Utility.Ensure3BitByte(type) << 4) | Utility.Ensure4BitByte(value)) };
    ...
    public static byte[] SongPositionPointer(int songPositionInMidiBeats)
       => new byte[] { (byte)SystemCommonStatus.SongPositionPointer, Utility.Ensure14BitLoByte(songPositionInMidiBeats), Utility.Ensure14BitHiByte(songPositionInMidiBeats) };
    public static byte[] SongSelect(int songNumber)
      => new byte[] { (byte)SystemCommonStatus.SongSelect, Utility.Ensure7BitByte(songNumber) };
