namespace Flux
{
  public static partial class SetOps
  {
    ...
    public static System.Collections.Generic.HashSet<T> Intersection<T>(this System.Collections.Generic.IEnumerable<T> source, System.Collections.Generic.IEnumerable<T> target, System.Collections.Generic.IEqualityComparer<T> equalityComparer)
    {
      if ((source is System.Collections.Generic.ICollection<T> sc && sc.Count == 0) || (target is System.Collections.Generic.ICollection<T> tc && tc.Count == 0))
        return new System.Collections.Generic.HashSet<T>(); // If either sequence is empty, the result is empty.

      var ths = new System.Collections.Generic.HashSet<T>(target, equalityComparer);

      if (!ths.Any()) return new System.Collections.Generic.HashSet<T>(); // If either sequence is empty, the result is empty.

      return new System.Collections.Generic.HashSet<T>(source.Where(s => ths.Contains(s)));
    }
