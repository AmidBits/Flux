...
namespace Flux.Text.PhoneticAlgorithm
{
    ...
    : IPhoneticAlgorithmEncoder
  {
    ...
    {
      return string.Concat(text.RemoveDiacriticalMarks(c => (char)ExtensionMethods.ReplaceDiacriticalLatinStroke((System.Text.Rune)c).Value).Where(c => GlobalizationEnUs.IsEnglishLetter((System.Text.Rune)c)).Select(c => char.ToUpper(c, System.Globalization.CultureInfo.CurrentCulture)));
    }
  }
}
