  ...
  namespace Geometry.Coordinates
  {
      ...
      : System.IFormattable
    {
      ...
      public string ToString(string? format, System.IFormatProvider? provider)
        => $"<{m_radius.ToString(format ?? 3.FormatUpToFractionalDigits(), provider)}, {m_inclination.ToUnitString(Quantities.AngleUnit.Degree, format ?? 6.FormatUpToFractionalDigits(), provider)} / {Elevation.ToUnitString(Quantities.AngleUnit.Degree, format ?? 6.FormatUpToFractionalDigits(), provider)}, {m_azimuth.ToUnitString(Quantities.AngleUnit.Degree, format ?? 6.FormatUpToFractionalDigits(), provider)}>";
