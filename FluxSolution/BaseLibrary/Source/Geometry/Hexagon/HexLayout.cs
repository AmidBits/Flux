namespace Flux.Geometry
{
  ...
  public record class HexLayout
  {
    ...
    private readonly CoordinateSystems.CartesianCoordinate2<int> m_origin;
    private readonly HexOrientation m_orientation;

    public HexLayout(HexOrientation orientation, Size2<int> size, CoordinateSystems.CartesianCoordinate2<int> origin)
    {
    ...
    public CoordinateSystems.CartesianCoordinate2<int> Origin { get => m_origin; init => m_origin = value; }
    ...
    public CoordinateSystems.HexCoordinate<double> PixelToHex(double x, double y)
    {
      ...
      return new CoordinateSystems.HexCoordinate<double>(q, r);
    }

    public void HexCornerOffset(int corner, out double x, out double y)
    {
      var angle = GenericMath.PiX2 * (m_orientation.StartAngle - corner) / 6.0;
