namespace Flux.Mechanics
{
    ...
    : ITrajectory2D
  {
    ...
    public DownhillTrajectory2D(Quantities.Length verticalDifference, Quantities.Angle initialAngle, Quantities.LinearVelocity initialVelocity, Quantities.Acceleration gravitationalAcceleration)
    {
    ...
    public DownhillTrajectory2D(Quantities.Length verticalDifference, Quantities.Angle initialAngle, Quantities.LinearVelocity initialVelocity)
    ...
    public Quantities.Length MaxHeight
      => new(m_verticalDifference.Value + System.Math.Pow(m_initialVelocity.Value, 2) * System.Math.Pow(System.Math.Sin(m_initialAngle.Value), 2) / (2 * m_gravitationalAcceleration.Value));
    ...
    public Quantities.Time MaxTime
      => new(m_initialVelocity.Value * System.Math.Sin(m_initialAngle.Value) / m_gravitationalAcceleration.Value + System.Math.Sqrt(2 * MaxHeight.Value / m_gravitationalAcceleration.Value));
