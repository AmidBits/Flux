namespace Flux.Geometry
{
  ...
  public readonly record struct TwoLineElementSet2
  {
    ...
    public override string ToString()
      => $"{GetType().Name} {{ Inclination = {new Units.Angle(m_radInclination).ToUnitValueString(Units.AngleUnit.Degree, QuantifiableValueStringOptions.Default)}, RightAscensionOfAscendingNode = {new Units.Angle(m_radRightAscensionOfAscendingNode).ToUnitValueString(Units.AngleUnit.Degree, QuantifiableValueStringOptions.Default)}, Eccentricity = {m_eccentricity}, ArgumentOfPerigee = {new Units.Angle(m_radArgumentOfPerigee).ToUnitValueString(Units.AngleUnit.Degree, QuantifiableValueStringOptions.Default)}, MeanAnomaly = {new Units.Angle(m_radMeanAnomaly).ToUnitValueString(Units.AngleUnit.Degree, QuantifiableValueStringOptions.Default)}, MeanMotion = {m_meanMotion} }}";
