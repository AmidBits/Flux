namespace Flux.Media
{
	public static class Acoustics
	{
		...
		public static double GetFrequency(double waveLength, double soundVelocity) => (soundVelocity / waveLength);
		/// <summary>Returns the wavelength in meters (m) from the specified sound velocity and frequency.</summary>
		public static double GetWaveLength(double soundVelocity, double frequency) => (soundVelocity / frequency);
		/// <summary>Returns the sound velocity in meters per second (m/s) from the specified frequency and wavelength.</summary>
		public static double GetSoundVelocity(double frequency, double waveLength) => (frequency * waveLength);
	}
	...
	public static class Frequency
	{
		...
		public static double Ratio(double frequencyA, double frequencyB) => (frequencyA / frequencyB);

    /// <summary>Convert a specified frequency ratio to cents.</summary>
    public static int FrequencyRatioToCents(double frequencyRatio) => (int)(System.Math.Log(frequencyRatio, 2.0) * 1200.0);
    ...
    public static double Normalized(double frequency, double sampleRate = 44100.0) => frequency / sampleRate;

    public static double RatioOfSemitones(int semitones) => Music.Interval.Cent.ToFrequencyRatio(semitones * 100);

    /// <summary>The number of samples in one complete frequency cycle.</summary>
    public static double SamplesPerCycle(double signalFrequency, double sampleRate = 44100.0) => sampleRate / signalFrequency;

    /// <summary>Computes the number of milliseconds per period of the specified frequency.</summary>
    public static double SecondsPerPeriod(double frequency) => 1.0 / frequency;

    /// <summary>Adjusts the pitch of the specified frequency, up or down, using a pitch interval specified in cents.</summary>
    public static double ShiftPitch(double frequency, int cents) => (System.Math.Pow(FrequencyRatioOfCent, cents) * frequency);
    ...
    public static double ToWaveLength(double frequency, double speedOfSound = 343.21) => speedOfSound / frequency;
  }
}
