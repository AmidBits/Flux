namespace Flux.Colors
{
    ...
    : System.IEquatable<Rgb>
  {
    ...
    [System.Runtime.InteropServices.FieldOffset(4)] private byte m_green;
    [System.Runtime.InteropServices.FieldOffset(8)] private byte m_blue;
    [System.Runtime.InteropServices.FieldOffset(12)] private byte? m_alpha;
    ...
    public static Rgb Random(System.Random rng)
    {
      ...
      return new Rgb(bytes[0], bytes[1], bytes[2], bytes[3]);
    }
    ...
    public double GetHue(out double chroma)
    {
      var max = System.Math.Max(System.Math.Max(Red, Green), Blue);
      var min = System.Math.Min(System.Math.Min(Red, Green), Blue);
      ...
      if (max == Red) h1 = (((Green - Blue) / chroma) + 6) % 6; // The % operator doesn't do proper modulo on negative numbers, so we'll add 6 before using it.
      else if (max == Green) h1 = ((Blue - Red) / chroma) + 2;
      else if (max == Blue) h1 = ((Red - Green) / chroma) + 4;

      return 60 * h1;
    }

    /// <summary>Converts the RGB color to grayscale, using plain average of all colors.</summary>
    public Rgb ToGrayscaleAverage()
    {
      var gray = System.Convert.ToByte((m_red + m_green + m_blue) / 3);

      return new Rgb(gray, gray, gray, Alpha);
    }
    /// <summary>Converts the RGB color to grayscale, using averages of the most prominent and least prominent colors.</summary>
    public Rgb ToGrayscaleLightness()
    {
      var gray = System.Convert.ToByte((System.Math.Max(System.Math.Max(m_red, m_green), m_blue) + System.Math.Min(System.Math.Min(m_red, m_green), m_blue)) / 2);

      return new Rgb(gray, gray, gray, Alpha);
    }
    /// <summary>Converts the RGB color to grayscale, using a weighted average based on human perception.</summary>
    public Rgb ToGrayscaleLuminosity()
    {
      var gray = System.Convert.ToByte(m_red * 0.21 + m_green * 0.72 + m_blue * 0.07);
    ...
    public Cmyk ToCmyk()
    {
      ...
      return new Cmyk(cyan, magenta, yellow, black, Alpha);
    }
    /// <summary>Creates an HSI color corresponding to the RGB instance.</summary>
    public Hsi ToHsi()
    {
      var rgb = (double)(Red + Green + Blue);
    ...
    public override int GetHashCode()
      => System.HashCode.Combine(Red, Green, Blue, Alpha);
    public override string ToString()
      => $"<{GetType().Name}: {Red}, {Green}, {Blue}, {Alpha}>";
  }
