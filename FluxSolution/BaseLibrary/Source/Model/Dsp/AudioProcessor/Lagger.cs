namespace Flux.Dsp.AudioProcessor
{
  public class LaggerMono : IAudioProcessorMono
  {
    ...
    public double Amount { get => m_amount; set => m_amount = Math.Clamp(value, 0.0, 1.0); }
    ...
    public ISampleMono ProcessAudio(ISampleMono sample) => new MonoSample(m_amount > Math.EpsilonCpp32 ? m_previousSample = Math.Interpolate.Cosine(sample.FrontCenter, m_previousSample, m_amount) : sample.FrontCenter);
  }
  ...
  public class LaggerStereo : IAudioProcessorStereo
  {
    ...
    public double Amount { get => Left.Amount; set => Right.Amount = Left.Amount = Math.Clamp(value, 0.0, 1.0); }
