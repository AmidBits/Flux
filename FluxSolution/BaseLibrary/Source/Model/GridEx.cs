...
namespace Flux.Model
{
  ...
  public class GridFixed<TValue> : IGrid<TValue>, System.ICloneable
  {
    ...
    {
      get => Items[CartesianToIndex(x, y, z)];
      set => Items[CartesianToIndex(x, y, z)] = value;
    }
    public TValue this[int x, int y]
    {
      get => Items[CartesianToIndex(x, y, 0)];
      set => Items[CartesianToIndex(x, y, 0)] = value;
    }
    ...
    public System.Collections.Generic.IEnumerable<int> Adjacent(int index)
    {
      ...
      for (int zi = (z > 0 ? z - 1 : z), zmax = (z < Length.Z - 1 ? z + 1 : z); zi <= zmax; zi++)
      {
        for (int yi = (y > 0 ? y - 1 : y), ymax = (y < Length.Y - 1 ? y + 1 : y); yi <= ymax; yi++)
        {
          for (int xi = (x > 0 ? x - 1 : x), xmax = (x < Length.X - 1 ? x + 1 : x); xi <= xmax; xi++)
          {
    ...
    public System.Collections.Generic.IEnumerable<int> Adjacent(int x, int y, int z) => Adjacent(CartesianToIndex(x, y, z));

    /// <summary>Converts an integer cartesian coordinate to a linear index, based on the x, y, z lengths.</summary>
    public int CartesianToIndex(int x, int y, int z) => x + y * Length.X + z * Length.X * Length.Y;
    ...
    {
      var irxy = index % Length.X * Length.Y;

      return (irxy % Length.X, irxy / Length.X, index / (Length.X * Length.Y));
    }

    public object Clone()
    {
      var clone = new GridFixed<TValue>(Length.X, Length.Y, Length.Z);

      System.Array.Copy(Items, clone.Items, Items.Length);
  ...
  public class SudokuGrid : GridFixed<string>
  {
    ...
    public static int ToIndex(string label) => (label.Length == 2 && RowLabels.Contains(label[0]) && ColumnLabels.Contains(label[1])) ? RowLabels.IndexOf(label[0]) * 9 + ColumnLabels.IndexOf(label[1]) : throw new System.ArgumentOutOfRangeException(nameof(label));
