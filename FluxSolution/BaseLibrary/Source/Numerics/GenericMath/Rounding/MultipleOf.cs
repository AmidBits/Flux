...
namespace Flux
{
  public static partial class GenericMath
  {
    ...
    /// <param name="properNearest">Proper means nearest but do not include x if it's a multiple-of, i.e. the two multiple-of will be properly nearest (but not the same), or LT/GT rather than LTE/GTE.</param>
    ...
    /// <summary>PREVIEW! Find the nearest (to <paramref name="x"/>) of two multiples, using the specified <see cref="FullRounding"/> <paramref name="mode"/>, and also return both multiples as out parameters.</summary>
    /// <param name="x">The value for which the nearest multiples of will be found.</param>
    /// <param name="multiple">The multiple to which the results will align.</param>
    /// <param name="properNearest">Proper means nearest but do not include x if it's a multiple-of, i.e. the two multiple-of will be properly nearest (but not the same), or LT/GT rather than LTE/GTE.</param>
    /// <param name="mode"></param>
    ...
    /// <param name="nearestAwayFromZero">Outputs the multiple of that is farther from zero.</param>
    /// <returns>The nearest two multiples to value.</returns>
    public static TSelf RoundToMultipleOf<TSelf>(this TSelf x, TSelf multiple, bool proper, FullRounding mode, out TSelf nearestTowardsZero, out TSelf nearestAwayFromZero)
      where TSelf : System.Numerics.INumber<TSelf>
    {
      GetNearestMultipleOf(x, multiple, proper, out nearestTowardsZero, out nearestAwayFromZero);
