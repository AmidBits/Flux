...
namespace Flux
{
  ...
  {
    /// <summary>
    /// <para>Creates a new DeBruijn sequence with DeBruijn numbers, which are the indices in a size <paramref name="k"/> alphabet (e.g. 10 digit number pad) of order-<paramref name="n"/> size (e.g. 4 digit codes).</para>
    /// <see href="https://www.rosettacode.org/wiki/De_Bruijn_sequences"/>
    /// </summary>
    ...
    public static System.Collections.Generic.IEnumerable<T> GetDeBruijnSequence<T>(int sizeK, int orderN, System.Collections.Generic.IList<T> alphabet)
      => sizeK <= alphabet.Count ? GetDeBruijnSequence(sizeK, orderN).Select(i => alphabet[i]) : throw new System.ArgumentOutOfRangeException(nameof(sizeK));

    /// <summary>Creates a new sequence with the code indices expanded.</summary>
    public static System.Collections.Generic.IEnumerable<System.Collections.Generic.List<T>> GetDeBruijnSequenceExpanded<T>(int sizeK, int orderN, System.Collections.Generic.IList<T> alphabet)
      => GetDeBruijnSequence(sizeK, orderN, alphabet).PartitionNgram(orderN, (e, i) => e.ToList());
  }
}
#endif
