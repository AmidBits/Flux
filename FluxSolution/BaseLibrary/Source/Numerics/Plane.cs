...
namespace Flux.Numerics
{
  ...
  public readonly record struct Plane
    : IPlane
  {
    private readonly static double NormalizeEpsilon = 1.192092896e-07f; // Smallest such that 1.0+NormalizeEpsilon != 1.0
    ...
    public Plane(Numerics.CartesianCoordinate4 normal, double distance)
    ...
    public void Deconstruct(out double x, out double y, out double z, out double distance) { x = m_x; y = m_y; z = m_z; distance = m_distance; }
    ...
    public double LengthSquared() => m_x * m_x + m_y * m_y + m_z * m_z;
    ...
    public Plane Normalize()
    {
      ...
      if (System.Math.Abs(ls - 1) < NormalizeEpsilon)
        return this; // It already normalized, so we don't need to further process.

      var invLen = 1 / System.Math.Sqrt(ls);

      return new(m_x * invLen, m_y * invLen, m_z * invLen, m_distance * invLen);
    }

    #region Static methods

    /// <summary>Creates a Plane that contains the three given points.</summary>
    public static Plane CreateFromVertices(Numerics.CartesianCoordinate4 point1, Numerics.CartesianCoordinate4 point2, Numerics.CartesianCoordinate4 point3)
    {
      var normal = Numerics.CartesianCoordinate4.Normalize(Numerics.CartesianCoordinate4.Cross(point2 - point1, point3 - point1));

      return new(normal, -Numerics.CartesianCoordinate4.EuclideanLengthSquared(normal));
    ...
    public static double Dot(Plane plane, Numerics.CartesianCoordinate4 value)
      => DotCoordinate(plane, value) * double.CreateChecked(value.W);
    ...
    public static double DotCoordinate(Plane plane, Numerics.CartesianCoordinate4 value)
    ...
    public static double DotNormal(Plane plane, Numerics.CartesianCoordinate4 value)
    ...
    public static Plane Transform(Plane plane, Numerics.IMatrix4 matrix)
    {
    ...
    public static Plane Transform(Plane plane, Numerics.Quaternion rotation)
    {
