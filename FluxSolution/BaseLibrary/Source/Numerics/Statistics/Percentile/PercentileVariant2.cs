namespace Flux.Statistics
{
    ...
    : IPercentileComputable
  {
    ...
    public static TPercent PercentileRank<TCount, TPercent>(TCount count, TPercent percent)
      ...
      => Quantities.UnitInterval.AssertMember(percent, IntervalNotation.Closed, nameof(percent)) * TPercent.CreateChecked(count.AssertNonNegativeRealNumber() - TCount.One) + TPercent.One;
      ...
      where TPercent : System.Numerics.IFloatingPoint<TPercent>
    {
      ...
      var x = PercentileRank(sampleCount, Quantities.UnitInterval.AssertMember(p, IntervalNotation.Closed, nameof(p)));
