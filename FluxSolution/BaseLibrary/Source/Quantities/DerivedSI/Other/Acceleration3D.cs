namespace Flux
{
  ...
  public record struct Acceleration3D
    : IUnitQuantifiable<CartesianCoordinate3<double>, AccelerationUnit>
  {
    private readonly CartesianCoordinate3<double> m_value;

    public Acceleration3D(CartesianCoordinate3<double> value, AccelerationUnit unit = Acceleration.DefaultUnit)
      => m_value = unit switch
      {
    ...
    [System.Diagnostics.Contracts.Pure]
    public CartesianCoordinate3<double> ToUnitValue(AccelerationUnit unit = Acceleration.DefaultUnit)
      => unit switch
      {
