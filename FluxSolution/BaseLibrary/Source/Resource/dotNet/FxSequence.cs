namespace Flux.Resources.DotNet
{
      ...
      : ITabularDataAcquirable
  {
    ...
    public static System.Collections.Generic.IEnumerable<object[]> GetData(System.Collections.IEnumerable enumerable)
      => enumerable.Cast<object>().Select(o => FxReflection.GetPropertyInfos(o).Select(pi => pi.GetValue(o)!).ToArray());

    #region Implemented interfaces

    public string[] FieldNames => FxReflection.GetPropertyInfos(GetData(m_enumerable).First()).Select(pi => pi.Name).ToArray();
    public System.Type[] FieldTypes => FxReflection.GetPropertyInfos(GetData(m_enumerable).First()).Select(pi => pi.PropertyType).ToArray();
