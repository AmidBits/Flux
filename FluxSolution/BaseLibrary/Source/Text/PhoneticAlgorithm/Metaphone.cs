...
namespace Flux.Text.PhoneticAlgorithm
{
    ...
    : IPhoneticAlgorithmEncoder
  {
    ...
    public string EncodePhoneticAlgorithm(System.ReadOnlySpan<char> expression)
    {
      ...
      __text = string.Concat(expression.ToString().RemoveUnicodeMarks().ReplaceUnicodeLatinStrokes().AsSpan().ToUpperCase(System.Globalization.CultureInfo.CurrentCulture).ToString().Where(c => char.IsLetter(c)));
      __pos = 0;
