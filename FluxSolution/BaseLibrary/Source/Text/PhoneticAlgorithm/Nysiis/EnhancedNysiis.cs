    ...
    : IPhoneticAlgorithmEncoder
  {
    ...
    public string EncodePhoneticAlgorithm(System.ReadOnlySpan<char> expression)
    {
      ...
      if (code.Length > 0)
      {
        ...
        var firstCharVowel = GlobalizationEnUs.IsEnglishVowel(code[0], false) ? code[0] : '\0'; // 1, if the first character of the name is a vowel, remember it.
        ...
        if (GlobalizationEnUs.IsEnglishVowel(firstCharVowel, false))
    ...
    public static string GetValidCharacters(string text)
      => string.Concat(text.RemoveDiacriticalMarks(c => ExtensionMethods.RemoveDiacriticalLatinStroke(c)).Where(c => GlobalizationEnUs.IsEnglishLetter(c)).Select(c => char.ToUpper(c, System.Globalization.CultureInfo.CurrentCulture)));
  }
}
