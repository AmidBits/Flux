...
namespace Flux.Text.PhoneticAlgorithm
{
    ...
    : IPhoneticAlgorithmEncoder
  {
    ...
    {
      return string.Concat(text.RemoveDiacriticalMarks(c => SystemCharEm.RemoveDiacriticalLatinStroke(c)).Where(c => GlobalizationEnUs.IsEnglishLetter(c)).Select(c => char.ToUpper(c, System.Globalization.CultureInfo.CurrentCulture)));
    }
  }
}
