namespace Flux
{
	public static partial class SystemTextEm
	{
		...
		public static bool IsWrapped(this System.Text.StringBuilder source, char left, char right)
			=> source is not null && source.Length >= 2 && source[0] == left && source[source.Length - 1] == right;
		...
		public static bool IsWrapped(this System.Text.StringBuilder source, string left, string right)
			=> source is not null && left is not null && right is not null && source.Length >= (left.Length + right.Length) && StartsWith(source, left) && EndsWith(source, right);
