...
namespace Flux.Text.Tokenization
{
    ...
    : IToken<TextElement>
  {
    ...
    public override string ToString()
      => $"<\"{Value}\" @{Index}{(string.Concat(GetNormalizationForms(Value.ToString(), false).Select((kvp, i) => $"[{kvp.Key}=\"{kvp.Value}\"]")) is var s && s.Length > 0 ? $" {s}" : string.Empty)}>";
  }
}
