...
namespace Flux
{
  ...
  public static partial class UnicodeEx
  {
    ...
    public static UnicodeCategoryMajorCode ParseMajorCode(string categoryMajor)
      => (UnicodeCategoryMajorCode)System.Enum.Parse(typeof(UnicodeCategoryMajorCode), categoryMajor);
    public static bool TryParseMajorCode(string categoryMajor, out UnicodeCategoryMajorCode result)
    {
      try
      {
        result = ParseMajorCode(categoryMajor);
        return true;
      }
...
#pragma warning restore CA1031 // Do not catch general exception types.
    ...
    public static bool TryParseMajorMinorCode(string categoryCode, out System.Globalization.UnicodeCategory result)
    {
      try
      {
        result = ParseMajorMinorCode(categoryCode);
        return true;
      }
#pragma warning disable CA1031 // Do not catch general exception types.
      catch { }
#pragma warning restore CA1031 // Do not catch general exception types.
    ...
    public static string ToStringLiteral(int codePoint)
      => codePoint < 0 ? throw new System.ArgumentOutOfRangeException(nameof(codePoint)) : codePoint <= 0xFFFF ? $@"\u{codePoint:x4}" : $@"\U{codePoint:X8}";

    public static int ParseUnicodeNotation(string text) => System.Text.RegularExpressions.Regex.Match(text, @"(?<=(U|u)\+)[0-9A-Fa-f]{4,}") is var m && m.Success && int.TryParse(m.Value, System.Globalization.NumberStyles.HexNumber, null, out var number) ? number : throw new System.ArgumentException(@"Could not parse unicode notation.");
...
#pragma warning restore CA1031 // Do not catch general exception types
    ...
    public static string ToUnicodeNotation(int codePoint)
      => $"U+{codePoint:X4}";
  }
}
