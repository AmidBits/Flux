namespace Flux
{
  namespace Units
  {
      ...
      where TDistribution : IValueQuantifiable<double>
    {
      ...
      public string ToString(string? format, IFormatProvider? formatProvider) => $"{m_amount.ToString(format, formatProvider)} per unit {m_distribution.ToString(format, formatProvider)} = {m_amount.Value / m_distribution.Value} {m_amount.ToString(format, formatProvider).Split(' ')[1]}/{m_distribution.ToString(format, formatProvider).Split(' ')[1]}";
    }
  }
}
