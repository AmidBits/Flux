namespace Flux
{
  public static partial class Fx
  {
    ...
    /// </summary>
    /// <remarks>Since an array is arbitrary in terms of e.g. rows and columns, we just adopt a this view, so we'll consider dimension 0 as the row dimension and dimension 1 as the column dimension.</remarks>
    public static T[,] InsertToCopy<T>(this T[,] source, ArrayDimension dimension, int index, int count) => source.InsertToCopy((int)dimension, index, count);
    ...
    /// <param name="count">The number of strands to add for the dimension.</param>
    /// <param name="pattern">The items to fill at index. Using a sort of continuous flow fill.</param>
    public static T[,] InsertToCopy<T>(this T[,] source, ArrayDimension dimension, int index, int count, params T[] pattern) => source.InsertToCopy((int)dimension, index, count, pattern);
  }
}
