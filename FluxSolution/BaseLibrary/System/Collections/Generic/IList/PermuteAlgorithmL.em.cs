namespace Flux
{
  public static partial class Permutation
  {
    ...
    public static System.Collections.Generic.IEnumerable<TSource[]> PermuteAlgorithmL<TSource>(this System.Collections.Generic.IList<TSource> source, System.Collections.Generic.IComparer<TSource>? comparer = null)
    {
      comparer ??= System.Collections.Generic.Comparer<TSource>.Default;

      var initialOrder = new int[source.Count]; // Figure out where we are in the sequence of all permutations
      for (var i = 0; i < source.Count; i++)
        initialOrder[i] = i;

      System.Array.Sort(initialOrder, (x, y) => comparer.Compare(source[x], source[y]));

      var transform = new System.ValueTuple<int, int>[source.Count];
      for (var i = 0; i < source.Count; i++)
        transform[i] = (initialOrder[i], i);

      for (var i = 1; i < source.Count; i++) // Handle duplicates
      ...
      while (true)
      {
        ...
        var decreasingPart = source.Count - 2;
        ...
        System.Array.Reverse(transform, decreasingPart + 1, source.Count - decreasingPart - 1); // Reverse the decreasing partition.
      }
