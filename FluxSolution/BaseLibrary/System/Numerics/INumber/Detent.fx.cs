namespace Flux
{
  public static partial class Maths
  {
    ...
    /// <summary>
    /// <para>Snaps the <paramref name="number"/> to zero if it's within the specified <paramref name="proximity"/> of zero, otherwise unaltered.</para>
    /// </summary>
    /// <remarks>This is similar to a knob that has a notch which latches the knob at the zero position.</remarks>
    public static TNumber DetentZero<TNumber>(this TNumber number, TNumber proximity)
      where TNumber : System.Numerics.INumber<TNumber>
      => TNumber.Zero.EqualsWithinAbsoluteTolerance(number, proximity)
      ? TNumber.Zero // Detent to zero.
      : number;
  }
}
