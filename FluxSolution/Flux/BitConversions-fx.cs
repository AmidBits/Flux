  ...
  public static partial class BitConversions
  {
      ...
      where T : System.Numerics.IBinaryInteger<T>
    {
      if (endianess == Endianess.LittleEndian)
        return T.TryReadLittleEndian(buffer, typeof(T).IsNumericsIUnsignedNumber(), out value);
      else if (endianess == Endianess.BigEndian)
        return T.TryReadBigEndian(buffer, typeof(T).IsNumericsIUnsignedNumber(), out value);
      else
      {
        value = default!;
        return false;
