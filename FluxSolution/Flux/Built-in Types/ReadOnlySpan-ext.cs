namespace Flux
{
  public static partial class ReadOnlySpanExtensions
  {
    ...
    extension<T>(System.ReadOnlySpan<T> source)
    {
      ...
      public bool ContainsAny(System.Func<T, bool> predicate, out int index) => ContainsAny(source, (e, i) => predicate(e), out index);
      ...
      /// <param name="predicate"></param>
      /// <returns></returns>
      public int CountBy(System.Func<T, int, bool> predicate)
      {
        var count = 0;
      ...
      /// <param name="predicate"></param>
      /// <returns></returns>
      public int CountBy(System.Func<T, bool> predicate) => CountBy(source, (e, i) => predicate(e));
      ...
      /// <param name="predicate"></param>
      /// <returns></returns>
      public int IndexOf(System.Func<T, bool> predicate) => IndexOf(source, (e, i) => predicate(e));
      ...
      /// <para><see href="https://en.wikipedia.org/wiki/Palindrome"/></para>
      /// </summary>
      /// <param name="equalityComparer">If null, then <see cref="System.Collections.Generic.EqualityComparer{T}.Default"/> is used.</param>
      /// <returns></returns>
      ...
      /// <summary>
      /// <para>A pangram is a sentence using every letter of a given alphabet at least once.</para>
      /// <para><see href="https://en.wikipedia.org/wiki/Pangram"/></para>
      /// </summary>
      ...
      #region ..Ordered.. (IComparer)

      public void AssertOrderedAscending(System.Collections.Generic.IComparer<T>? comparer)
      {
        if (!IsOrderedAscending(source, comparer))
          throw new System.ArgumentException("The elements must be in ascending order.");
      }

      public void AssertOrderedDescending(System.Collections.Generic.IComparer<T>? comparer)
      {
        if (!IsOrderedDescending(source, comparer))
          throw new System.ArgumentException("The elements must be in descending order.");
      }

      public bool IsOrderedAscending(System.Collections.Generic.IComparer<T>? comparer)
      {
        if (source.Length <= 1)
          return true;
      ...
      public bool IsOrderedDescending(System.Collections.Generic.IComparer<T>? comparer)
      {
        if (source.Length <= 1)
          return true;
      ...
      /// <summary>
      /// <para>Indicates whether a <see cref="System.ReadOnlySpan{T}"/> is wrapped in the specified left and right <typeparamref name="T"/>. E.g. brackets, or parenthesis.</para>
      /// </summary>
      ...
      /// <summary>
      /// <para>Indicates whether a <see cref="System.ReadOnlySpan{T}"/> is wrapped in the specified left and right readonly-spans.</para>
      /// </summary>
      ...
      /// <param name="predicate"></param>
      /// <returns></returns>
      public int LastIndexOf(System.Func<T, bool> predicate) => LastIndexOf(source, (e, i) => predicate(e));
      ...
      #region ShortestRepeatingSubstring
    ...
    extension(System.ReadOnlySpan<byte> source)
    {
      #region ReadBits

      /// <summary>
      /// <para>Reads <paramref name="count"/> bits starting at <paramref name="index"/> from a byte array.</para>
      /// </summary>
      /// <typeparam name="T"></typeparam>
      /// <param name="index">The starting bit index (0 = first bit of first byte).</param>
      /// <param name="count">The number of bits to read (limit depends on the size of <typeparamref name="T"/>).</param>
      /// <returns></returns>
      public TInteger ReadBits<TInteger>(int index, int count)
        where TInteger : System.Numerics.IBinaryInteger<TInteger>
      {
        TInteger bitBuffer = TInteger.Zero;
        ...
        var prefixBits = 8 - bitOffset;
        var byteLength = (count - prefixBits) / 8;
        var suffixBits = count - (byteLength * 8) - prefixBits;
      ...
      /// <summary>
      /// <para>Attempts to read <paramref name="count"/> bits starting at <paramref name="index"/> from a byte array.</para>
      /// </summary>
      /// <typeparam name="T"></typeparam>
      /// <param name="index">The starting bit index (0 = first bit of first byte).</param>
      /// <param name="count">The number of bits to read (limit depends on the size of <typeparamref name="T"/>).</param>
      /// <param name="bitsRead">The bits read from the byte array.</param>
      /// <returns></returns>
      public bool TryReadBits<TInteger>(int index, int count, out TInteger bitsRead)
        where TInteger : System.Numerics.IBinaryInteger<TInteger>
      {
        try
        {
          bitsRead = ReadBits<TInteger>(source, index, count);
          return true;
        }
        catch { }

        bitsRead = TInteger.Zero;
        return false;
      }
    ...
    extension(System.ReadOnlySpan<char> source)
    {
      ...
      /// <summary>
      /// <para>Indicates whether the character sequence is all whitespace.</para>
      /// </summary>
      /// <returns></returns>
      ...
      where TNumber : System.Numerics.INumber<TNumber>
    {
      ...
      /// <param name="target"></param>
      /// <returns></returns>
      public double GetInterpolatedIndex(TNumber target)
      {
        if (source.IsEmpty || !source.IsOrderedAscending())
          return double.NaN;
        ...
        if (target < source[lo])
          return double.NegativeInfinity; // If target is larger than the max-value of the array.

        if (target > source[hi])
          return double.PositiveInfinity; // If target is smaller than the max-value of the array.

        while (hi - lo > 1) // Loop until narrowed down to two neighbors.
        {
          if (TNumber.IsZero(source[hi] - source[lo]))
            return lo; // All values same in range.

          var position = lo + (double.CreateChecked(target - source[lo]) * (hi - lo) / double.CreateChecked(source[hi] - source[lo]));

          var index = (int)double.Floor(position);
        ...
        return lo + double.CreateChecked(target - source[lo]) / double.CreateChecked(source[hi] - source[lo]); // Final fractional position between low and high.
      }
