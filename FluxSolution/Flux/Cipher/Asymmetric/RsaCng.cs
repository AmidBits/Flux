namespace Flux.Cipher.Asymmetric
{
  public sealed class RsaCng
    : IAsymmetricCipher
  {
    public static IAsymmetricCipher Default { get; } = new RsaCng();
    ...
    public static (byte[] PrivateKey, string PrivateText, byte[] PublicKey, string PublicText) CreateRsaKeyPair(int keySizeInBits, int splitLineLength = 64)
    {
      ...
      var privateLines = BinaryInteger.GenerateSubRangesBySubLength(privateB64.Length, splitLineLength).Select(r => privateB64[r]).ToList();
      ...
      var publicLines = BinaryInteger.GenerateSubRangesBySubLength(publicB64.Length, splitLineLength).Select(r => publicB64[r]).ToList();
