namespace Flux.DataStructures.Immutable
{
    ...
    : IBinaryTree<TValue>
  {
    ...
    /// <returns></returns>
    public static IBinaryTree<TValue> Create(TValue[] inOrder, TValue[] levelOrder)
    {
      return ConstructTree(ImmutableBinaryTree<TValue>.Empty, levelOrder, inOrder, 0, inOrder.Length - 1);

      static DataStructures.IBinaryTree<TValue> ConstructTree(Flux.DataStructures.IBinaryTree<TValue> startNode, TValue[] levelOrder, TValue[] inOrder, int inStart, int inEnd)
      {
        if (inStart > inEnd)
