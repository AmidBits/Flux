namespace Flux.DataStructures
{
  public static partial class Traversal
  {
    ...
    public static System.Collections.Generic.IEnumerable<T> BinaryTreeSearchDfsR<T>(T node, System.Func<T, T> selectorChildLeft, System.Func<T, T> selectorChildRight, BinaryTrees.BinaryTreeDepthOrder order, System.Func<T, bool>? predicate = null)
    {
      ...
      if (order == BinaryTrees.BinaryTreeDepthOrder.PreOrder)
        yield return node;

      if (order == BinaryTrees.BinaryTreeDepthOrder.ReverseInOrder)
      ...
      if (order == BinaryTrees.BinaryTreeDepthOrder.InOrder || order == BinaryTrees.BinaryTreeDepthOrder.ReverseInOrder)
        yield return node;
      ...
      if (order == BinaryTrees.BinaryTreeDepthOrder.PostOrder)
        yield return node;
    }
