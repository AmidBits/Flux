namespace Flux.Dsp.WaveProcessors
{
    ...
    : IMonoWaveProcessable
  {
    ...
    public double Release { get => m_release; set => m_release = double.Clamp(value, Tools.PositiveThreshold, 1.0); }
    ...
    public double ProcessAudioMono(double sample)
    {
      ...
      {
        m_autoAttenuation = double.Clamp(m_autoAttenuation + m_release, Tools.PositiveThreshold, 1.0);
      }
    ...
    public double ProcessMonoWave(double wave)
    {
      if (m_autoAttenuation < 1.0)
      {
        m_autoAttenuation = double.Clamp(m_autoAttenuation + m_release, Tools.PositiveThreshold, 1);
      }
