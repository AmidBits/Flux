    ...
    : IMonoWaveProcessable
  {
    ...
    public double Multiplier
    {
      ...
      set
      {
        ...
        if (m_multiplier > Tools.PositiveThreshold)
          m_multiplier = m_multiplier * 9.0 + 1.0;
        else if (m_multiplier < Tools.NegativeThreshold)
          m_multiplier += 1.0;
      }
    }
    ...
    public double ProcessMonoWave(double wave)
      => Number.FoldAcross(m_multiplier * (wave + m_polarBias), -1, 1);
    ...
    public static double ApplyFolder(double sample, double polarBias, double multiplier)
      => Number.FoldAcross(multiplier * (sample + polarBias), -1, 1);
  }
}
