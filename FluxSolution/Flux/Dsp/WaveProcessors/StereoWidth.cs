    ...
    : IStereoWaveProcessable
  {
    ...
    public double Width
    {
      ...
      set
      {
        ...
        m_stereoCoefficient = (m_width > Tools.PositiveThreshold || m_width < Tools.NegativeThreshold) ? (m_width + 1) / 2 : 1;
      }
    }
    ...
    public static (double left, double right) Apply(double width, double left, double right)
      => (left + right) / 2 is var m && (right - left) * (width > Tools.PositiveThreshold || width < Tools.NegativeThreshold ? (width + 1) / 2 : 1) is var s ? (m - s, m + s) : (left, right);
  }
}
