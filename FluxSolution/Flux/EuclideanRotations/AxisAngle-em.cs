namespace Flux
{
  ...
  {
    public static EuclideanRotations.EulerAngles ToEulerAngles(this EuclideanRotations.AxisAngle source)
    {
    ...
    public static (CoordinateSystems.CartesianCoordinate axis, Units.Angle angle) ToQuantities(this EuclideanRotations.AxisAngle source)
      => (
        new((float)source.X.Value, (float)source.Y.Value, (float)source.Z.Value),
        source.Angle
      );

    public static System.Numerics.Quaternion ToQuaternion(this EuclideanRotations.AxisAngle source)
    {
      var (angle, x, y, z) = source;

      (x, y, z, var w) = EuclideanRotations.AxisAngle.ConvertAxisAngleToQuaternion(angle, x, y, z);
