namespace Flux.Geometry.CoordinateSystems
{
    ...
    : System.IFormattable
  {
    ...
    public PolarCoordinate(double radiusMeter, double azimuthRadian)
      : this(radiusMeter, Units.LengthUnit.Meter, azimuthRadian, Units.AngleUnit.Radian) { }
    ...
    {
      var (x, y) = likeCompass ? ConvertPolarToCartesian2Ex(m_radius.Value, m_azimuth.Value) : ConvertPolarToCartesian2(m_radius.Value, m_azimuth.Value);

      return new(x, y, 0, 0);
    }

    public Shapes.Circle.CircleGeometry ToCircleGeometry() => new(m_radius.Value);
    ...
    public string ToString(string? format, System.IFormatProvider? provider)
      => $"<{m_radius.ToString(format ?? 3.FormatUpToFractionalDigits(), provider)}, {m_azimuth.ToUnitString(Units.AngleUnit.Degree, format ?? 6.FormatUpToFractionalDigits(), provider)}>";
