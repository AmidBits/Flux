namespace Flux.Geometry.Coordinates
{
    ...
    : System.IFormattable
  {
    ...
    public SphericalCoordinate(double radiusMeter, double inclinationRadian, double azimuthRadian)
      : this(radiusMeter, Units.LengthUnit.Meter, inclinationRadian, Units.AngleUnit.Radian, azimuthRadian, Units.AngleUnit.Radian) { }
    ...
    public double SphereSurfaceArea => Units.Area.OfSphere(m_radius.Value);
    ...
    public CylindricalCoordinate ToCylindricalCoordinate()
    {
      ...
      return new(
        r * si, Units.LengthUnit.Meter,
        m_azimuth.Value, Units.AngleUnit.Radian,
        r * ci, Units.LengthUnit.Meter
      );
    }
    ...
    public GeographicCoordinate ToGeographicCoordinate()
      => new(
        m_inclination.Value - double.Pi / 2, Units.AngleUnit.Radian,
        m_azimuth.Value, Units.AngleUnit.Radian,
        m_radius.Value, Units.LengthUnit.Meter
      );
    ...
    public string ToString(string? format, System.IFormatProvider? provider)
      => $"<{m_radius.ToString(format ?? 3.FormatUpToFractionalDigits(), provider)}, {m_inclination.ToUnitString(Units.AngleUnit.Degree, format ?? 6.FormatUpToFractionalDigits(), provider)} / {Elevation.ToUnitString(Units.AngleUnit.Degree, format ?? 6.FormatUpToFractionalDigits(), provider)}, {m_azimuth.ToUnitString(Units.AngleUnit.Degree, format ?? 6.FormatUpToFractionalDigits(), provider)}>";
