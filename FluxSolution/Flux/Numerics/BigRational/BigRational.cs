namespace Flux.Numerics
{
    ...
    , Units.IValueQuantifiable<double>
  {
    ...
    public bool IsDyadicFraction
      => System.Numerics.BigInteger.IsPow2(m_denominator);
    ...
    public bool IsProperFraction
      => !m_numerator.IsZero && System.Numerics.BigInteger.Abs(m_numerator) < m_denominator; // If the numerator is not zero and strictly less than the denominator, it is a proper fraction.

    /// <summary>Determines whether <paramref name="value"/> represents a mixed numbered fraction.</summary>
    public bool IsMixedFraction
      => System.Numerics.BigInteger.Abs(m_numerator) > m_denominator;
    ...
    public BigRational PartProperFraction
      => new(System.Numerics.BigInteger.Remainder(m_numerator, m_denominator), m_denominator, false);

    public System.Numerics.BigInteger PartWholeNumber
    ...
    public string ToMixedFractionString(UnicodeSpacing spacing = UnicodeSpacing.NarrowNoBreakSpace)
      => $"{PartWholeNumber}{(char)spacing}{PartProperFraction.ToFractionString()}";
    ...
    private static BigRational NthRoot(BigRational value, int nth, BigRational maxError)
    {
      ...
      static (System.Numerics.BigInteger value, bool isExact) IntegerNthRoot(System.Numerics.BigInteger a, System.Numerics.BigInteger n)
      {
        ...
        var upperPow = IBinaryInteger.Pow(upperX, n);
        ...
        while (upperPow.CompareTo(a) < 0)
        {
          ...
          upperPow = IBinaryInteger.Pow(upperX, n);
        }
        ...
        while (true)
        {
          ...
          var testPow = IBinaryInteger.Pow(testX, n);
    ...
    public string ToString(string? format, System.IFormatProvider? formatProvider)
      => IsProperFraction
      ? ToFractionString() //Units.Ratio.ToRatioNotationString(Units.RatioNotation.AslashB, m_numerator.ToString(format, formatProvider), m_denominator.ToString(format, formatProvider))
      : IsMixedFraction // TryGetMixedParts(this, out var wholeNumber, out var properNumerator, out var properDenominator)
      ? ToMixedFractionString() //$"{wholeNumber} {Units.Ratio.ToRatioNotationString(Units.RatioNotation.AslashB, properNumerator.ToString(format, formatProvider), properDenominator.ToString(format, formatProvider))}"
