namespace Flux.Numerics.Geometry.Hexagons
{
    ...
    : System.IFormattable, IFigurable
  {
    ...
    public Polygons.PolygonRegularConvex ToPolygonPointyTopped() => Polygons.PolygonRegularConvex.Create(6, m_sideLength);
    ...
    public static double ConvertInradiusToCircumradius(double inradius) => inradius / RadiusRatio;
    ...
    public static double ConvertCircumradiusToInradius(double circumradius) => circumradius * RadiusRatio;
    ...
    /// <param name="y"></param>
    /// <returns></returns>
    public static bool HexagonContainsPoint(double sideLength, double x, double y)
    {
      ...
      if (q2x > h || q2y > 2 * v) return false; // Bounding test (since q2 is in quadrant 2, only 2 tests are needed).

      return (2 * v * h) - (q2x * v) - (q2y * h) >= 0; // Finally the dot product can be reduced to this, due to the hexagon symmetry.
    }
