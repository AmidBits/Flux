namespace Flux
{
  public static partial class BinaryInteger
  {
      ...
      where TInteger : System.Numerics.IBinaryInteger<TInteger>
    {
      ...
      if (value < TInteger.CreateChecked(59))
        return value.NaiveFactorial();

      return value.SplitFactorial();

    }

    //public static TInteger OddFactorial<TInteger>(this TInteger value)
    // where TInteger : System.Numerics.IBinaryInteger<TInteger>
    //{
    //  System.ArgumentOutOfRangeException.ThrowIfNegative(value);
    ...
    //  TInteger f = TInteger.One;

    //  var two = TInteger.CreateChecked(2);
    //  var three = TInteger.CreateChecked(3);

    //  for (var i = TInteger.One; i < value - TInteger.One; i++)
    //  {
    //    two.TryFastIntegerPow(i + TInteger.One, out var tz, out var afz, out var _);

    //    f *= OddProduct(three, tz - TInteger.One);
    //  }

    //  return two.FastIntegerPow(two.FastIntegerPow(value - TInteger.One, out var _).ipowTz, out var _).ipowTz * f;

    //  //  2 **(2 **k - 1) * f

    //  TInteger OddProduct(TInteger l, TInteger h)
    //  {
    //    var p = TInteger.One;

    //    var two = TInteger.One << 1;

    //    var ml = (l % two > TInteger.Zero) ? l : (l + TInteger.One);
    //    var mh = (h % two > TInteger.Zero) ? h : (h - TInteger.One);
    ...
    /// <returns></returns>
    public static TInteger NaiveFactorial<TInteger>(this TInteger value)
      where TInteger : System.Numerics.IBinaryInteger<TInteger>
    {
      System.ArgumentOutOfRangeException.ThrowIfNegative(value);
