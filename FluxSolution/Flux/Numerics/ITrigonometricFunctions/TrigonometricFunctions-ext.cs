namespace Flux
{
  public static partial class TrigonometricFunctions
  {
      ...
      where TFloat : System.Numerics.IFloatingPoint<TFloat>, System.Numerics.ITrigonometricFunctions<TFloat>
    {
      ...
      public TFloat Atan2Ccw(TFloat x)
      {
        ...
        if (atan2 < 0)// The positive range is already 0..+Pi, if so, no adjustments needed.
          atan2 = (atan2 + double.Tau) % double.Tau; // Adjust the negative portion of atan2, from -Pi..0 into +Pi..+Tau, which is just a matter of adding a full turn (Tau).
