namespace Flux.PhoneticAlgorithm
{
    ...
    : IPhoneticallyEncodable
  {
    ...
    public string EncodePhonetic(System.ReadOnlySpan<char> expression)
    {
      _output = new System.Text.StringBuilder();

      __text = string.Concat(expression.ToString().RemoveUnicodeMarks().ReplaceUnicodeLatinStrokes().AsSpan().ToUpper(System.Globalization.CultureInfo.CurrentCulture).ToString().Where(c => char.IsLetter(c)));
      __pos = 0;
    ...
    {
      System.ArgumentNullException.ThrowIfNull(characters);

      return characters.Contains(c, System.StringComparison.Ordinal);
    }
