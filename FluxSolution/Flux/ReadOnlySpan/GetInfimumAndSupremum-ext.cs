namespace Flux
{
  public static partial class XtensionReadOnlySpan
  {
    extension<T>(System.ReadOnlySpan<T> source)
    {
      ...
      /// <returns></returns>
      public (int InfimumIndex, T? InfimumItem, TValue? InfimumValue, int SupremumIndex, T? SupremumItem, TValue? SupremumValue) GetInfimumAndSupremum<TValue>(TValue referenceValue, System.Func<T, TValue> valueSelector, bool proper, System.Collections.Generic.IComparer<TValue>? comparer = null)
      {
        ...
        var infIndex = -1;
        var infItem = default(T);
        var infValue = referenceValue;

        var supIndex = -1;
        var supItem = default(T);
