        ...
        foreach (var line in reader.ReadLines(s => s.Length > 0, s => s))
        {
          if (line == @"=")
          {
            if (lines.IsCommonPrefix(0, @"KEY:"))
            {
              ...
              var key = dictionary.TryGetValue("key", out var lKey) ? lKey : [];
              var syn = dictionary.TryGetValue("syn", out var lSyn) ? lSyn : [];
              var ant = dictionary.TryGetValue("ant", out var lAnt) ? lAnt : [];

              yield return new string[][] { [.. key], [.. syn], [.. ant] };
            }
