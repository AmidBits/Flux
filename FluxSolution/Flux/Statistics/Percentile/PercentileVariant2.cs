namespace Flux.Statistics.Percentile
{
    ...
    : IPercentileComputable
  {
    ...
    public static TPercent PercentileRank<TCount, TPercent>(TCount count, TPercent percent)
      ...
      => Units.UnitInterval.AssertWithin(percent, IntervalNotation.Closed, nameof(percent)) * TPercent.CreateChecked(count.AssertNonNegativeNumber() - TCount.One) + TPercent.One;
      ...
      where TPercent : System.Numerics.IFloatingPoint<TPercent>
    {
      ...
      var x = PercentileRank(sampleCount, Units.UnitInterval.AssertWithin(p, IntervalNotation.Closed, nameof(p)));
