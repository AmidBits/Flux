namespace Flux
{
  ...
  namespace Text
  {
    ...
    {
      public static void Recode(this System.IO.Stream source, int sourceBitSize, System.IO.Stream target, int targetBitSize, int groupBitSize)
      {
        using var reader = new Flux.IO.BitStream.BitStreamReader(source);
        using var writer = new Flux.IO.BitStream.BitStreamReader(target);

        while (true)
        {
          var read = reader.ReadBitsUInt64(groupBitSize, out var actualRead);

          if (actualRead < groupBitSize)
            break;
        }
        ...
        where TTarget : System.Numerics.IBinaryInteger<TTarget>
      {
        ...
        while (sourceIndex < source.Length && targetIndex < target.Length)
        {
          ...
          target[targetIndex++] = TTarget.CreateChecked((sourceBitField >> (sourceBitCount - targetBitSize)) & IBinaryInteger.CreateBitMaskRight(((ulong)targetBitSize)));
        ...
        if (sourceBitCount > 0)
          target[targetIndex++] = TTarget.CreateChecked((sourceBitField << (targetBitSize - minCount)) & IBinaryInteger.CreateBitMaskRight(((ulong)minCount)));
      }
