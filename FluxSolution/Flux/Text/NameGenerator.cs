  ...
  namespace Text
  {
    ...
    public partial class NameGenerator
    {
      ...
      public System.Collections.Generic.Dictionary<string, System.Collections.Generic.Dictionary<string, double>> Construct_Chain(System.Collections.Generic.IList<string> list)
      {
        ...
        foreach (var i in Number.ArithmeticSequence(0, 1).Take(maxLength))
          cpl[(int)i + 1] = list.Where(n => i < n.Length).ToHistogram(k => k[(int)i], f => char.IsVowel(f[(int)i], System.Globalization.CultureInfo.CurrentCulture) ? 1 : 1).ToCumulativeDistributionFunction(1.0);
        ...
        {
          var names = SplitRegex().Split(list[i]);
      ...
      {
        if (!chain["table_len"].TryGetValue(key, out double value)) return null;
