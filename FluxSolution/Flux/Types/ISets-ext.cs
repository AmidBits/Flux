namespace Flux
{
  ...
  public static partial class XtensionISet
  {
    extension<T>(System.Collections.Generic.ISet<T> sources)
    {
      /// <summary>
      ...
      public (int unfoundCount, int uniqueCount) ScanSetCounts(System.Collections.Generic.IEnumerable<T> target, bool returnIfUnfound, System.Collections.Generic.IEqualityComparer<T>? equalityComparer = null)
      {
        ...
        {
          if (sources.Contains(item, equalityComparer)) unique.Add(item);
      ...
      /// <summary>Adds all elements from <paramref name="other"/> to the current <see cref="System.Collections.Generic.List{T}"/>.</summary>
      public void AddRange(System.Collections.Generic.IEnumerable<T> other) => AddRange(sources, other, out var _);
      ...
      /// <summary>Adds all elements from <paramref name="other"/> to the current <see cref="System.Collections.Generic.ISet{T}"/> and returns the number of elements that were successfully added.</summary>
      public void AddSpan(System.ReadOnlySpan<T> other) => AddSpan(sources, other, out var _);
      ...
      public void RemoveSpan(System.ReadOnlySpan<T> other) => RemoveSpan(sources, other, out var _);
      ...
      {
        if (ReferenceEquals(sources, target))
        ...
        if (ths.Count == 0)
          return sources; // If target is empty, source is the result.

        var ihs = new System.Collections.Generic.HashSet<T>(sources.Intersect(ths));

        return sources.Except(ihs).Concat(ths.Except(ihs));
      }
    }
  }
}