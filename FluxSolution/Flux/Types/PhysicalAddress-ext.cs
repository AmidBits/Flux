namespace Flux
{
  public static class PhysicalAddressExtensions
  {
    extension(System.Net.NetworkInformation.PhysicalAddress source)
    {
      ...
      public System.ReadOnlySpan<byte> CreateMagicPacket(byte[]? secureOn = default)
      {
      ...
      /// <param name="secureOn">Certain NICs support a security feature called "SecureOn". It allows users to store within the NIC a hexadecimal password of 6 bytes. Clients have to append this password to the magic packet. Any network eavesdropping will expose the cleartext password.</param>
      public void SendWakeOnLan(System.Net.IPEndPoint broadcastAddress, byte[]? secureOn = default)
      {
        System.ArgumentNullException.ThrowIfNull(source);

        using var udpClient = new System.Net.Sockets.UdpClient();

        udpClient.Send(CreateMagicPacket(source, secureOn), broadcastAddress);
      }
      ...
      /// <param name="secureOn">Certain NICs support a security feature called "SecureOn". It allows users to store within the NIC a hexadecimal password of 6 bytes. Clients have to append this password to the magic packet. Any network eavesdropping will expose the cleartext password.</param>
      public void SendWakeOnLanIPv4(System.Net.IPAddress ipAddress, System.Net.IPAddress subnetMask, int port = 0, byte[]? secureOn = default)
        => SendWakeOnLan(source, new System.Net.IPEndPoint(ipAddress.GetBroadcastAddressIPv4(subnetMask), port), secureOn);
