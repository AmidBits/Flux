namespace Flux.Units
{
  ...
  public readonly record struct Azimuth
    : System.IComparable, System.IComparable<Azimuth>, System.IFormattable, Units.IValueQuantifiable<double>
  {
    ...
    private Azimuth(double degrees)
    ...
    /// </summary>
    public Azimuth(double angle, Units.AngleUnit unit = Units.AngleUnit.Degree)
      => m_degrees = IntervalNotation.HalfOpenRight.WrapAroundNative(Angle.ConvertUnit(angle, unit, AngleUnit.Degree), MinValue, MaxValue);
    ...
    public PlanetaryScience.CompassRose32Wind CompassAbbreviation => CompassPoint(m_degrees, PlanetaryScience.PointsOfTheCompass.ThirtytwoWinds, MidpointRounding.ToZero, out double _);
    ...
    /// <returns></returns>
    public static PlanetaryScience.CompassRose32Wind CompassPoint(double azimuth, PlanetaryScience.PointsOfTheCompass precision, System.MidpointRounding roundingMethod, out double notch)
      => (PlanetaryScience.CompassRose32Wind)(int)((notch = LatchNeedle(azimuth, (int)precision, roundingMethod)) * (32 / (int)precision));
    ...
    public static Azimuth ParseAbbreviation(string compassPointAbbreviated)
      => System.Enum.TryParse<PlanetaryScience.CompassRose32Wind>(compassPointAbbreviated.Trim(), true, out var thirtytwoWindCompassRose)
    ...
    {
      var wordsOfTheCompassPoints = System.Enum.GetNames<PlanetaryScience.WordsOfTheCompass>();
    ...
    public static Azimuth operator +(Azimuth a, Azimuth b) => a + b.m_degrees;
    public static Azimuth operator /(Azimuth a, double b) => new(a.m_degrees / b);
    public static Azimuth operator /(Azimuth a, Azimuth b) => a / b.m_degrees;
    public static Azimuth operator *(Azimuth a, double b) => new(a.m_degrees * b);
    public static Azimuth operator *(Azimuth a, Azimuth b) => a * b.m_degrees;
    ...
    public override string ToString() => ToString(BinaryInteger.CreateFormatStringWithCountDecimals(3), null); // Up to three decimals.
  }
}
