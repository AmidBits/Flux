namespace Flux.Units
{
    ...
    : System.IComparable, System.IComparable<Irradiance>, System.IFormattable, ISiUnitValueQuantifiable<double, IrradianceUnit>
  {
    ...
    public Irradiance(MetricPrefix prefix, double wattPerSquareMeter) => m_value = prefix.ConvertPrefix(wattPerSquareMeter, MetricPrefix.Unprefixed);
    ...
    public string ToSiUnitString(MetricPrefix prefix, string? format = null, System.IFormatProvider? formatProvider = null)
      => GetSiUnitValue(prefix).ToSiFormattedString(format, formatProvider) + UnicodeSpacing.ThinSpace.ToSpacingString() + prefix.GetMetricPrefixSymbol() + IrradianceUnit.WattPerSquareMeter.GetUnitSymbol();
    ...
    public string ToUnitString(IrradianceUnit unit = IrradianceUnit.WattPerSquareMeter, string? format = null, System.IFormatProvider? formatProvider = null, UnicodeSpacing spacing = UnicodeSpacing.Space, bool fullName = false)
    {
      var value = GetUnitValue(unit);

      return value.ToString(format, formatProvider) + UnicodeSpacing.Space.ToSpacingString() + (fullName ? unit.GetUnitName(INumber.IsConsideredPlural(value)) : unit.GetUnitSymbol(false));
    }
