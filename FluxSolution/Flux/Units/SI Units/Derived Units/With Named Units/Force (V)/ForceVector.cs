namespace Flux.Units
{
    ...
    : System.IFormattable, ISiUnitValueQuantifiable<System.Runtime.Intrinsics.Vector256<double>, ForceUnit>
  {
    ...
    public ForceVector(MetricPrefix prefix, System.Runtime.Intrinsics.Vector256<double> NewtonSquare) => m_value = prefix.ConvertPrefix(NewtonSquare, MetricPrefix.Unprefixed);
    ...
    public string ToSiUnitString(MetricPrefix prefix, string? format = null, System.IFormatProvider? formatProvider = null)
      => GetSiUnitValue(prefix).ToString() + UnicodeSpacing.ThinSpace.ToSpacingString() + prefix.GetMetricPrefixSymbol() + ForceUnit.Newton.GetUnitSymbol();
    ...
    public static System.Runtime.Intrinsics.Vector256<double> GetUnitFactor(ForceUnit unit) => System.Runtime.Intrinsics.Vector256.Create(unit.GetUnitFactor());

    public static string GetUnitName(ForceUnit unit, bool preferPlural) => unit.ToString().ToPluralUnitName(preferPlural);

    public static string GetUnitSymbol(ForceUnit unit, bool preferUnicode) => unit.GetUnitSymbol(preferUnicode);

    public System.Runtime.Intrinsics.Vector256<double> GetUnitValue(ForceUnit unit) => ConvertToUnit(unit, m_value);

    public string ToUnitString(ForceUnit unit = ForceUnit.Newton, string? format = null, System.IFormatProvider? formatProvider = null, UnicodeSpacing spacing = UnicodeSpacing.Space, bool fullName = false)
      => GetUnitValue(unit).ToString() + spacing.ToSpacingString() + (fullName ? GetUnitName(unit, true) : GetUnitSymbol(unit, false));
