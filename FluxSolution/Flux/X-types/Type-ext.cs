namespace Flux
{
  public static partial class TypeExtensions
  {
    ...
    extension(System.Type type)
    {
      ...
      public bool IsAssignableToGenericTypee(Type generic)
      {
        return new[] { type }.Concat(type.GetType().GetInterfaces()).Any(i => i.IsConstructedGenericType && i.GetGenericTypeDefinition() == generic) || (type.GetType().BaseType?.IsAssignableToGenericTypee(generic) ?? false);
      }
      ...
      public bool IsAssignableToGenericType(System.Type genericType)
        => type.GetInterfaces().Prepend(type).Any(i => i.IsGenericType && i.GetGenericTypeDefinition() == genericType) || (type.BaseType?.IsAssignableToGenericType(genericType) ?? false);
      //{
      ...
      /// <returns></returns>
      public bool IsNumericsIBinaryInteger(bool isPrimitive = false)
      {
        TryGetSystemNullable(type, out type);

        return type is not null && (!isPrimitive || type.IsPrimitive) && type.IsAssignableToGenericType(typeof(System.Numerics.IBinaryInteger<>));
      }
      ...
      /// <param name="isPrimitive"></param>
      /// <returns></returns>
      public bool IsNumericsIFloatingPoint(bool isPrimitive = false)
      {
        TryGetSystemNullable(type, out type);

        return type is not null && (!isPrimitive || type.IsPrimitive) && type.IsAssignableToGenericType(typeof(System.Numerics.IFloatingPoint<>));
      }
      ...
      /// <param name="isPrimitive"></param>
      /// <returns></returns>
      public bool IsNumericsINumber(bool isPrimitive = false)
      {
        TryGetSystemNullable(type, out type);

        return type is not null && (!isPrimitive || type.IsPrimitive) && type.IsAssignableToGenericType(typeof(System.Numerics.INumber<>));
      }
      ...
      /// <param name="isPrimitive"></param>
      /// <returns></returns>
      public bool IsNumericsISignedNumber(bool isPrimitive = false)
      {
        TryGetSystemNullable(type, out type);

        return type is not null && (!isPrimitive || type.IsPrimitive) && type.IsAssignableToGenericType(typeof(System.Numerics.ISignedNumber<>));
      }
      ...
      /// <param name="isPrimitive"></param>
      /// <returns></returns>
      public bool IsNumericsIUnsignedNumber(bool isPrimitive = false)
      {
        TryGetSystemNullable(type, out type);

        return type is not null && (!isPrimitive || type.IsPrimitive) && type.IsAssignableToGenericType(typeof(System.Numerics.IUnsignedNumber<>));
      }
      ...
      /// <summary>
      /// <para>Indicates whether a <see cref="System.Type"/> is System.Nullable<T>.</para>
      /// </summary>
      ...
      /// <returns></returns>
      public bool TryGetSystemNullable(out System.Type underlyingType)
      {
        if (IsSystemNullable(type))
        {
