namespace FluxNet.Numerics
{
  public static partial class Maths
  {
    ...
    /// </remarks>
    public static TNumber FastIntegerLog<TNumber, TRadix>(this TNumber number, TRadix radix, MidpointRounding mode, out double log)
      where TNumber : System.Numerics.INumber<TNumber>
      where TRadix : System.Numerics.IBinaryInteger<TRadix>
    {
      checked
      {
        log = TNumber.IsZero(number) ? 0.0 : double.Log(double.CreateChecked(TNumber.Abs(number)), double.CreateChecked(AssertRadix(radix)));

        return (log < 1.0) ? TNumber.Zero : TNumber.CopySign(TNumber.CreateChecked(double.Round(log, mode)), number);
      }
    }
  }
}
