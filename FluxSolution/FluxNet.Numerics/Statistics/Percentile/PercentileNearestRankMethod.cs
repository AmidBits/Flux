namespace FluxNet.Numerics.Statistics.Percentile
{
    ...
    : IPercentileComputable
  {
    ...
    public static TPercent PercentileRank<TCount, TPercent>(TCount count, TPercent percent)
      ...
      => Extensions.AssertPercent(percent) * TPercent.CreateChecked(count.AssertNonNegativeInteger());
    ...
    public static TPercent PercentileScore<TScore, TPercent>(System.Collections.Generic.IEnumerable<TScore> distribution, TPercent p)
      ...
      where TPercent : System.Numerics.IFloatingPoint<TPercent>
      => TPercent.CreateChecked(distribution.ElementAt(System.Convert.ToInt32(TPercent.Ceiling(PercentileRank(distribution.Count(), p))) - 1));
