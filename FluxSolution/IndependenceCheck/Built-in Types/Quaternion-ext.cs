namespace Flux
{
  ...
  {
    extension(System.Numerics.Quaternion source)
    {
      /// <summary></summary>
      /// <remarks>The quaternion must be normalized.</remarks>
      public Numerics.EuclideanRotations.AxisAngle ToAxisAngle()
      {
      ...
      public Numerics.EuclideanRotations.EulerAngles ToEulerAngles() // yaw (Z), pitch (Y), roll (X)
      {
        var x = source.X;
        var y = source.Y;
        var z = source.Z;
