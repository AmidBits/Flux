namespace Flux.Model.Sudoku
{
  /// <summary>
  /// <para></para>
  /// <para><see href="http://www.norvig.com/sudoku.html"/></para>
  /// <para><see href="https://github.com/norvig/pytudes/blob/main/ipynb/Sudoku.ipynb"/></para>
  /// </summary>
  public static partial class SudokuConstraintPropagation
  {
    public static int[] ColumnLabelsInt = [1, 2, 3, 4, 5, 6, 7, 8, 9];
    public static int[] RowLabelsInt = [1, 2, 3, 4, 5, 6, 7, 8, 9];
    ...
    ////=> Squares.Select(i => System.Linq.Enumerable.Empty<System.Collections.Generic.List<int>>().Append(GetUnitColumn(i).ToList()).Append(GetUnitRow(i).ToList()).Append(GetUnitBox(i).ToList()).ToList()).ToList();
    ...
    public static System.Collections.Generic.List<string>? Assign(System.Collections.Generic.List<string> values, int square, int digit)
    {
      System.ArgumentNullException.ThrowIfNull(values);

      if (values[square].Select(c => c - '0').Where(c => c != digit).Any(c => Eliminate(values, square, c) == null))
        return null;
    ...
    public static System.Collections.Generic.List<string>? Eliminate(System.Collections.Generic.List<string> values, int square, int digit)
    {
      System.ArgumentNullException.ThrowIfNull(values);

      if (!values[square].Select(c => c - '0').Contains(digit))
      {
        return values; // already eliminated
      }

      values[square] = values[square].Replace(digit.ToString(), string.Empty, System.StringComparison.Ordinal);

      #region 0 or 1: if a square is reduced to only one possible digit (d), then eliminate d from the square's peers

      if (values[square].Length == 0) // contradiction, the last digit was just removed
      {
        return null;
      }
      else if (values[square].Length == 1) // if a square is reduced to only one possible digit (d), then eliminate d from the square's peers.
      {
        var d = values[square][0] - '0';

        if (Utilities.SquarePeers[square].Any(s => Eliminate(values, s, d) == null))
        {
          return null;
        }
      }
      ...
      foreach (var unit in Utilities.SquareUnits[square])
      {
        var places = unit.Where(s => values[s].Contains(digit.ToString(), System.StringComparison.Ordinal));
    ...
    {
      var values = Utilities.Squares.Select(i => ColumnLabels).ToList();

      foreach (var (square, character) in Utilities.Squares.Zip(puzzle, (s, c) => (square: s, character: c)))
      {
        if (char.IsDigit(character))
        {
          if (char.IsDigit(character) && Assign(values, square, character - '0') == null)
          {
            yield return null;
            yield break;
          }
    ...
    public static System.Collections.Generic.List<string>? Search(System.Collections.Generic.List<string>? values)
    {
      ...
      return values[square].Select(digit => Search(Assign(values.ToList(), square, digit - '0'))).FirstOrDefault(d => d != null);
    }
    ...
    public static System.Collections.Generic.List<string> Create(System.Random rng, int count = 17)
    {
      ...
      var squares = Utilities.Squares.ToList();
      ...
      foreach (var square in squares.RandomElements(.23, rng))
      {
        ...
        if (values.Where(s => s.Length == 1) is var ones && ones.Count() == count && ones.Distinct().Count() >= 8)
    ...
    public static string ToConsoleString(System.Collections.Generic.List<string> values)
    {
      ...
      foreach (var r in SudokuConstraintPropagation.RowLabels)
      {
        foreach (var c in SudokuConstraintPropagation.ColumnLabels)
        {
          sb.Append(new System.Text.StringBuilder(values[SudokuConstraintPropagation.ToIndex(r.ToString() + c.ToString())]).PadEven(width, ' ', ' '));
