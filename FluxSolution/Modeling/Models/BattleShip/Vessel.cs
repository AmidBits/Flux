...
namespace Flux
{
  public static partial class VesselEm
  {
    public static string ToConsoleString(this System.Collections.Generic.IList<Model.BattleShip.Vessel> ships, Size2<int> size, int screenPositionLeft, int screenPositionTop)
    {
      ...
      foreach (Model.BattleShip.Vessel s in ships)
        foreach (CoordinateSystems.CartesianCoordinate2<int> p in s.Locations)
  ...
  namespace Model.BattleShip
  {
    ...
    {
      private readonly System.Collections.Generic.List<CoordinateSystems.CartesianCoordinate2<int>> m_positions;

      public VesselOrientation m_orientation;

      public Vessel(int length, CoordinateSystems.CartesianCoordinate2<int> location, VesselOrientation orientation)
      {
        if (length <= 1) throw new System.ArgumentOutOfRangeException(nameof(length));

        m_orientation = orientation;

        m_positions = new System.Collections.Generic.List<CoordinateSystems.CartesianCoordinate2<int>>();

        for (int i = 0; i < length; i++)
          m_positions.Add(orientation == VesselOrientation.Horizontal ? new CoordinateSystems.CartesianCoordinate2<int>(location.X + i, location.Y) : new CoordinateSystems.CartesianCoordinate2<int>(location.X, location.Y + i));
      }

      public int Length
        => m_positions.Count;

      public CoordinateSystems.CartesianCoordinate2<int> Location
        => m_positions[0];
      public System.Collections.Generic.IReadOnlyList<CoordinateSystems.CartesianCoordinate2<int>> Locations
        => m_positions;
      ...
      public static bool AnyHitsOn(Vessel ship, System.Collections.Generic.IEnumerable<CoordinateSystems.CartesianCoordinate2<int>> shots)
        => ship.m_positions.Any(location => shots.Any(shot => shot == location));
      public static bool AnyHits(System.Collections.Generic.IEnumerable<Vessel> ships, System.Collections.Generic.IEnumerable<CoordinateSystems.CartesianCoordinate2<int>> shots)
        => ships.Any(ship => AnyHitsOn(ship, shots));
      public static bool IsSunk(Vessel ship, System.Collections.Generic.IEnumerable<CoordinateSystems.CartesianCoordinate2<int>> shots)
        => ship.m_positions.All(l => shots.Any(s => s == l));

      public static bool AreAdjacent(Vessel a, Vessel b)
      {
        foreach (CoordinateSystems.CartesianCoordinate2<int> p in a.Locations)
        {
          if (Intersects(b, new CoordinateSystems.CartesianCoordinate2<int>(p.X + 1, p.Y + 0)))
            return true;
          if (Intersects(b, new CoordinateSystems.CartesianCoordinate2<int>(p.X + -1, p.Y + 0)))
            return true;
          if (Intersects(b, new CoordinateSystems.CartesianCoordinate2<int>(p.X + 0, p.Y + 1)))
            return true;
          if (Intersects(b, new CoordinateSystems.CartesianCoordinate2<int>(p.X + 0, p.Y + -1)))
            return true;
        }
      ...
      public static bool Intersects(Vessel ship, CoordinateSystems.CartesianCoordinate2<int> position)
      {
      ...
      public static System.Collections.Generic.List<Vessel> StageFleet(Size2<int> gridSize, params int[] shipSizes)
      {
        ...
        foreach (var size in shipSizes)
        {
          ...
          do
          {
            ship = new Vessel(size, new CoordinateSystems.CartesianCoordinate2<int>(Random.NumberGenerators.Crypto.Next(gridSize.Width), Random.NumberGenerators.Crypto.Next(gridSize.Height)), (VesselOrientation)Random.NumberGenerators.Crypto.Next(2));
          }
