...
namespace Flux.Model
{
  ...
  public static class Chess // Correlated from top/left to bottom/right.
  {
    ...
    {
      var (x, y) = CartesianConversion.LinearIndexToCartesian2(index, BoardSize.X);
    ...
    public static int LabelToIndex(string column, string row)
    {
      ...
      return (int)CartesianConversion.Cartesian2ToLinearIndex(x, y, BoardSize.X);
    }
    ...
    public static int VectorToIndex(System.Drawing.Point vector)
      => (int)CartesianConversion.Cartesian2ToLinearIndex(vector.X, vector.Y, BoardSize.X);
