  ...
  public sealed class GameOfLife
  {
    ...
    public GameOfLife(System.Drawing.Point cellGrid, bool canLifeLogicWrapAroundEdges, double probabilityOfBeingInitiallyAlive)
    {
      ...
      for (var r = m_cellGrid.Y - 1; r >= 0; r--)
      {
        ...
        {
          var index = CartesianConversion.Cartesian2ToLinearIndex(c, r, m_cellGrid.X);
    ...
    private int CountLiveNeighbors(int x, int y)
    {
      ...
      for (var r = -1; r <= 1; r++) // Loop "rows".
      {
        ...
        for (var c = -1; c <= 1; c++) // Loop "columns".
        {
          ...
          var pointIndex = CartesianConversion.Cartesian2ToLinearIndex(x1, y1, m_cellGrid.X);

          cn += m_deadOrAlive[(int)pointIndex] ? 1 : 0;
        }
      }

      var positionIndex = CartesianConversion.Cartesian2ToLinearIndex(x, y, m_cellGrid.X);
    ...
    public System.Collections.Generic.IEnumerable<string> ToConsoleStrings()
    {
      for (var y = 0; y < m_cellGrid.Y; y++)
      {
        ...
        {
          var index = CartesianConversion.Cartesian2ToLinearIndex(x, y, m_cellGrid.X);
