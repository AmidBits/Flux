  ...
  public sealed class GameOfLife
  {
    ...
    public GameOfLife(Geometry.CartesianCoordinate2<int> cellGrid, bool canLifeLogicWrapAroundEdges, double probabilityOfBeingInitiallyAlive)
    {
      ...
      for (var r = m_cellGrid.Y - 1; r >= 0; r--)
      {
        ...
        {
          var index = (int)Convert.Cartesian2ToMapIndex(c, r, m_cellGrid.X);
    ...
    private int CountLiveNeighbors(int x, int y)
    {
      ...
      for (var r = -1; r <= 1; r++) // Loop "rows".
      {
        ...
        for (var c = -1; c <= 1; c++) // Loop "columns".
        {
          ...
          var pointIndex = (int)Convert.Cartesian2ToMapIndex(x1, y1, m_cellGrid.X);

          cn += m_deadOrAlive[pointIndex] ? 1 : 0;
        }
      }

      var positionIndex = (int)Convert.Cartesian2ToMapIndex(x, y, m_cellGrid.X);
    ...
    public System.Collections.Generic.IEnumerable<string> ToConsoleStrings()
    {
      for (var y = 0; y < m_cellGrid.Y; y++)
      {
        ...
        {
          var index = (int)Convert.Cartesian2ToMapIndex(x, y, m_cellGrid.X);
