...
namespace Flux.Model.MineSweeper
{
  public sealed class GameState
  {
    ...
    public Numerics.CartesianCoordinate2<int> CursorPosition { get; set; }

    private GameState(IImmutableStack<Covers> moves, Numerics.CartesianCoordinate2<int> cursorPosition)
    {
      m_moves = moves;
    ...
    public static GameState Create(Covers covers)
      => new(ImmutableStack.Create(covers), new Numerics.CartesianCoordinate2<int>(0, 0));

    public GameState Do(Covers covers)
      => new(m_moves.Push(covers), CursorPosition);

    public GameState Do(Numerics.CartesianCoordinate2<int> cursorPosition)
