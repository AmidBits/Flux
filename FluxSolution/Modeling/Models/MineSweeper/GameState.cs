...
namespace Flux.Model.MineSweeper
{
  public sealed class GameState
  {
    ...
    public GridCoordinate2 CursorPosition { get; set; }

    private GameState(IImmutableStack<Covers> moves, GridCoordinate2 cursorPosition)
    {
      m_moves = moves;
    ...
    public static GameState Create(Covers covers)
      => new(ImmutableStack.Create(covers), new GridCoordinate2(0, 0));

    public GameState Do(Covers covers)
      => new(m_moves.Push(covers), CursorPosition);

    public GameState Do(GridCoordinate2 cursorPosition)
