...
namespace Wormhole
{
		...
		: Core.IDataSource
	{
		...
		System.Collections.Generic.IEnumerable<object[]> Preset2Steps(string preset, string[] properties)
		{
			...
			var groupsDN = Query("(&(objectCategory=group)(member=*))", properties).Select(a => a[0].ToString()).ToArray();
			...
			System.Collections.Generic.IEnumerable<object[]> Query(string queryFilter, params string[] queryProperties)
			{
				...
				{
					yield return queryProperties.Length > 0
						? queryProperties.Select(qp => searchResult.Properties[qp][0]).ToArray()
						: (new object[] { searchResult.Path.Substring(7) });
				}
			}
		}
